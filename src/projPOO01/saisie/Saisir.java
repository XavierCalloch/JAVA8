...
public class Saisir {
	...
	public static ArrayList<Personne> listeInterimaires = new ArrayList<Personne>();
	...
	/** Méthode qui permet de saisir un intérimaire
	 * 
	 */
	public static void SaisirInterimaire() {

		ArrayList<Interimaire> listeInter = new ArrayList<Interimaire>();
		String nom, prenom, adresse, ville, codepostal, contratAgence = "", secu = null;
		double salaire = 0;
		int duree = 0;
		boolean erreurcp = true;
		boolean erreurns = true;
		...
		for (int i = 0; i < 2; i++) {
			...
			contratAgence = "";
			System.out.println("Saisir le contrat de l'agence de l'intérimaire");
			while (Menus.sc.hasNext() && contratAgence == "") {
			...
			listeInter.add(inter);
		}

		listeInterimaires = new ArrayList<Personne>(listeInter);
		if (Menus.choixmenu != 1) {
			Menus.Menu();
		}
		return;
